...
export class UsuarioController extends BaseController {
  ...
  async listar(@Query() paginacionQueryDto: FiltrosUsuarioDto) {
    ...
    this.logger.debug('usuarios>>>', result)
  ...
  async reenviarCorreoActivacionPorCorreo(
    @Body() correoDto: { correo: string }
  ) {
    const { correo } = correoDto
    const result =
      await this.usuarioService.reenviarCorreoActivacionPorCorreo(correo)
    return this.successUpdate(result, Messages.SUCCESS_RESEND_MAIL_ACTIVATION)
  }
}
