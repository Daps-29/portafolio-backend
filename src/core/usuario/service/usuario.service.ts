...
export class UsuarioService extends BaseService {
  ...
  async crearCuenta(usuarioDto: CrearUsuarioCuentaDto) {
    this.logger.info(`🔐 Crear cuenta de usuario`, usuarioDto)
    // verificar si el usuario ya fue registrado con su correo
    const usuario = await this.usuarioRepositorio.buscarUsuario(
      usuarioDto.correoElectronico
    )

    const buscarPorNombreUsuario = await this.usuarioRepositorio.buscarUsuario(
      usuarioDto.usuario
    )
    if (buscarPorNombreUsuario) {
      throw new PreconditionFailedException('Ya existe el nombre de usuario.')
    }
      ...
      const personaNueva = await this.personaRepositorio.crear(
        {
          ...
          fechaNacimiento: usuarioDto.persona.fechaNacimiento || new Date(),
  ...
  async recuperarCuenta(recuperarCuentaDto: RecuperarCuentaDto) {
    ...
    const datosCorreo = {
      correo: recuperarCuentaDto.correoElectronico,
      asunto: Messages.SUBJECT_EMAIL_ACCOUNT_RECOVERY,
      contenido: template,
    }
  ...
  async enviarCorreoContrasenia(
    ...
    const datosCorreoConContenido = {
      ...datosCorreo,
      contenido: template,
    }

    // const result = await this.emailService
    //   .sendEmail(datosCorreoConContenido)
    //   .catch((error) => {
    //     const mensaje = `Ocurrió un error al enviar el correo electrónico`
    //     this.logger.error(error, mensaje)
    //   })
    // return result
  }
  ...
  async reenviarCorreoActivacionPorCorreo(correo: string) {
    const usuario = await this.usuarioRepositorio.buscarUsuarioPorCorreo(correo)
    const statusValid = [Status.PENDING]
    ...
    return { id: usuario.id, estado: usuarioResult.estado }
  }
}
