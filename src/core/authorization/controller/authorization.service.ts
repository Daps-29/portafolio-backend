...
export class AuthorizationService extends BaseService {
  ...
  const politicas = await this.enforcer.getPolicy()
  ...
  await this.enforcer.addPolicy(sujeto, objeto, accion, app)
    return politica
  }
  ...
  async eliminarPolitica(politica: PoliticaDto) {
    const { sujeto, objeto, accion, app } = politica
  await this.enforcer.removePolicy(sujeto, objeto, accion, app)
    return politica
  }

  async obtenerRoles() {
  return await this.enforcer.getFilteredPolicy(3, 'frontend')
  }

  async obtenerPermisosPorRol(rol: string) {
  const politicas = await this.enforcer.getFilteredPolicy(
      3,
      'frontend'
    )
